...
def train(
    ...
    for _ in range(epochs):
        ...
        for _, data in enumerate(train_loader):
            inputs, labels = data[0].float(), data[1]  # labels is the true classfication
            optimizer.zero_grad()
            outputs = model(inputs)            # outputs is the predict value

            loss = criterion(y_pred=outputs, y_true=labels)  # using criterion to evaluate loss
            loss.backward()
            optimizer.step()
        ...
        res['train'].append(running_loss / total)
        ...
        if val_loader != None:
            ...
            for _, data in enumerate(val_loader):
                ...
                total += labels.shape[0]
            res["val"].append(running_loss_val / total)
    return res
