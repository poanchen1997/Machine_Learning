...
def crossentropy_parameter_search(
    ...
    res = {}
    softmodel = SoftmaxLayer()

    # Linear regression model
    model = softmodel(LinearLayer(2, 2))
    cri = CrossEntropyLossLayer()
    # print(list(model.parameters))
    opti = SGDOptimizer(model.parameters, lr=1e-4)
    Linear = train(train_loader, model, cri, opti, val_loader, epochs=10)
    res["Linear"] = {"train": Linear["train"], "val": Linear["val"], "model": LinearLayer}
