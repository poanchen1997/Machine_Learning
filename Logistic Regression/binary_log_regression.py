...
class BinaryLogReg:
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        # raise NotImplementedError("Your Code Goes Here")
        return np.mean(np.log(1 + np.exp(-y * (self.bias + X @ self.weight)))) + self._lambda * (self.weight.T @ self.weight)
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        # raise NotImplementedError("Your Code Goes Here")
        return np.mean((-y * (1 - self.mu(X, y)))[:, None] * X, axis=0) + 2 * self._lambda * self.weight
    ...
    def train(
        ...
        """
        num_batches = int(np.ceil(len(X_train) // batch_size))  # what is this for ?
        ...
        if method == "GD":
            while np.linalg.norm(self.weight - w_prev, np.inf) >= 1e-4 and loop_time <= epochs:
                loop_time += 1
